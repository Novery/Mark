...
+ int到char  1 + "0" -> 1 -> '1'
+ Character.toLowerCase(char) 大转小
+ char++ a->b
+ Math.pow(2,4); 2^4
+ string.matches("[A-Za-z//+//-]")
+ 排序
+ Collections.sort(list);
+ stream().sorted().findFirst(); 正序
  sorted(Collectors.reverseOrder());
 倒排+ 数据已经是正序
     + 数据已经是倒序 所有元素都相同
+ 栈，stack，.empty() .push .pop
+ 斐波那契数列
+ 快排 取首位为哨兵， 从首位末位开始循环比较元素，递归， 
+ 前序遍历，中序遍历，后序遍历
   + 先输出该节点，再输出左孩子，最后输出它的右孩子（前序遍历第一个为根节点）
   + 先输出左孩子，再输出该节点，最后输出它的右孩子（根节点在差不多中间位置，左边为左子树，右边为右子树）
   + 先输出左孩子，再输出右孩子，最后输出该节点（根节点在结尾）
   + 递归 Arrays.copybuRange复制数组
+ B树是否是A树的子树，先看两个节点本身，如果不相等，则递归判断A树左节点，右节点，找到对应的根节点， 比较过程，左节点遍历完成了则为true，左节点没遍历完成右节点遍历完成返回false，两个节点比较值不一样返回false，递归看对应的左节点右节点
+ 链表倒序，需要节点本身，上一个节点，下一个节点
+ 单调不减，单调递增，正相关,链表合并，两个节点比较大小，选择大的或小的节点排在前，使他的next指向递归函数
+ 顺时针打印，记录left，right,top，bottom,注意加时不要重复计算，除了首尾间转角处的，还有最后终点，需要判断当前left=right，则不加其中一个，top=bottom不算其中一个
+ stack.top取出最小元素，应用一个辅助栈，压的时候，如果A栈的压入比B栈压入大，B栈不压，，，，小于等于，AB栈同时压入，出栈，如果，AB栈顶元素不等，A出，B不出
+ 判断两个数组，一个是否为另一个栈的弹出序列，模拟入栈出栈过程，需要一个辅助栈，遍历塞入栈，每次塞入后遍历出栈消费，如果有对应值，出栈，index加一【相当于出栈的数组对应值消费掉了】，最后判断栈是否空了，
+ http https
  + http明文， https使用ssl，证书鉴权，同时加密
+ HTTP 应用层，无状态  tcp传输层，有状态
+ 正向代理 反向代理  代理客户端，发送到服务端  vpn  反向代理，代理的是服务器

+ redis快，多路I/O复用epoll（epoll 是只轮询那些真正发出了事件的流）,非阻塞，单线程，内存，避免了不必要上下文切换。

+ AOP的实现方式:JDK动态代理和CGLIB
+ 如果目标对象实现了接口，默认情况下会采用JDK的动态代理实现AOP
+ 如果目标对象实现了接口，可以强制使用CGLIB实现AOP。

+ 主存（可以类比为堆） 线程副本（可以类比为栈）和物理计算机发展史有关，cpu-》cache->内存，cache会存在不一致的情况，像sychronized、jvm也是通过JMM规范定义的，解决这个缓存不一致问题的
+ mysql在update时，先记日志
  + 首先日志的作用是为了在crash掉时做恢复的
  + 这种模式是因为，传统的机械硬盘，是有转速的概念的，日志记录是直接连续空间，update需要找到需要更新数据的位置，是分散的，失败的概率更高
+ linux操作系统如何保证 内核空间和用户空间

+ jdk1.6 sychronized锁升级，偏向锁，轻量级锁，再到重量级锁，
  + 偏向锁，偏向第一个获取它的线程，如果没被其他线程获取，无需同步
  + 比较当前线程id是否与Markword当中的线程id一致
  + 如果一致，继续执行
  + 如果不一致，是否偏向，未偏向则用CAS竞争锁，将线程id插入Markword中，同时改变偏向锁标志位
  + 如果已偏向说明存在竞争，此时升级为轻量锁

+ mysql调优 profile  performance schema ，定位一条语句的I/O消耗和CPU消耗
+ type ALL全表扫描，index遍历索引树查询，range在一个索引上进行范围查找
